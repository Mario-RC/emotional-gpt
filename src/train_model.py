...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    if args.local_rank in [-1, 0]:
        if SummaryWriter is None:
            logger.warning(
                "TensorBoard is not installed. Continuing without TensorBoard logging. "
...
def main():
    ...
    parser.add_argument("--line_by_line", action="store_true", help="Whether distinct lines of text in the dataset are to be handled as distinct sequences.")
    ...
    if args.block_size <= 0:
        args.block_size = tokenizer_max_length
        # Our input block size will be the max possible for the model
    else:
        args.block_size = min(args.block_size, tokenizer_max_length)
